...
@Component
public class OpenApiDeclarationRoute extends StdJsonProducerEntityRoute {
    ...
    protected Optional<?> doRoute(RestxRequest restxRequest, RestxRequestMatch match, Object body) throws IOException {
        ...
        return Optional.of(ImmutableMap.builder()
                ...
                .put("basePath", "http:" + restxRequest.getBaseNetworkPath())
    ...
    private List<ResourceDescription> filteredApis(List<ResourceDescription> apis) {
        return FluentIterable.from(apis)
                .transform(new Function<ResourceDescription, ResourceDescription>() {
                    @Override
                    public ResourceDescription apply(ResourceDescription input) {
                        if (input.operations != null) {
                            input.operations = FluentIterable.from(input.operations)
                                    .filter(new Predicate<OperationDescription>() {
                                        @Override
                                        public boolean apply(OperationDescription input) {
                                            return "GET".equalsIgnoreCase(input.httpMethod);
                                        }
                                    }).toList();
                        }
                        return input;
                ...
                .filter(new Predicate<ResourceDescription>() {
                    @Override
                    public boolean apply(ResourceDescription input) {
                        return !input.path.contains("@") && input.operations != null && !input.operations.isEmpty();
                    }
                })
                .toList();
    }
